...
void GetDMX()
{
   if (DMXSerial.receive())
   {
		...
		tempProgram = map(DMXSerial.read(DMX_Channel + 1), 0, 255, 0, maxProgram);
		currSpeed = map(DMXSerial.read(DMX_Channel + 2), 0, 255, 0, 100);
    	currFade = 255 - DMXSerial.read(DMX_Channel + 3);
